function GestionFlota({ currentUser = {}, medicion = null, vehiculos = [], medidores = [] }) {
  ...
  const renderMedidor = medidor => {
    ...
    return React.createElement('div', { key: medidor.id, style: { borderTop: '1px solid var(--line)', padding: '10px 0', display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center', opacity: medidor.activo === false ? .55 : 1 } },
  ...
  const renderVehiculo = vehiculo => React.createElement('div', { key: vehiculo.id, style: { borderTop: '1px solid var(--line)', padding: '10px 0', display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center', opacity: vehiculo.activo === false ? .55 : 1 } },
  ...
  const renderMedidorForm = () => medidorForm && React.createElement('div', { style: { background: 'var(--surface-2)', borderRadius: 9, padding: 10, marginTop: 10 } },
  ...
  const renderVehiculoForm = () => vehiculoForm && React.createElement('div', { style: { background: 'var(--surface-2)', borderRadius: 9, padding: 10, marginTop: 10 } },
  ...
  return React.createElement('div', { style: { padding: '16px 12px 28px', color: 'var(--ink)' } },
    ...
    React.createElement('div', { style: { background: 'var(--info-bg)', border: '1px solid var(--line)', borderRadius: 9, padding: 10, fontSize: 11, lineHeight: 1.45, marginBottom: 14 } }, equivalencia, ' La lectura física inicial y final pertenece a la jornada; el incremento entre clientes es lógico y calculado.'),
    React.createElement('section', { style: estiloSeccion }, React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginBottom: 8 } }, React.createElement('h3', { style: { margin: 0, fontSize: 16 } }, 'Medidores de flujo'), React.createElement('button', { type: 'button', onClick: () => iniciarMedidor(null), style: estiloBoton }, '+ Registrar medidor')), React.createElement('div', { style: { fontSize: 11, color: 'var(--ink-faint)', marginBottom: 10 } }, 'La existencia del medidor se registra aquí; su historial de lecturas se crea exclusivamente desde Jornada.'), medidoresVista, renderMedidorForm()),

function FlujoChoferRapido({
  ...
  const clientesFiltrados = clientesLocalidad.filter(c => {
    ...
    return !q || `${c.nombre || ''} ${c.domicilio || c.direccion || ''} ${c.localidadNombre || c.localidad || ''}`.toLowerCase().includes(q);
  });
  ...
  return React.createElement('div', { className: 'fx-page-route fx-page-operacion fx-page-operacion-driver', style: { padding: '14px 12px 28px' } },
    React.createElement('div', { className: 'fx-operacion-route-heading', style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 } },
      React.createElement('div', null, React.createElement('div', { style: { fontSize: 21, fontWeight: 800, letterSpacing: '-.02em' } }, 'Operación'), React.createElement('div', { style: { fontSize: 12, color: color.soft, marginTop: 3 } }, localidad)), React.createElement('div', { style: { fontSize: 11, color: color.soft, textAlign: 'right' } }, 'PASO ', paso, ' DE ', pasosTotales, React.createElement('div', { style: { marginTop: 5, height: 4, width: 72, background: color.line, borderRadius: 5, overflow: 'hidden' } }, React.createElement('div', { style: { height: '100%', width: `${Math.min(100, paso / pasosTotales * 100)}%`, background: color.accent } })))
    ),

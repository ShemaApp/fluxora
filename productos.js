...
function Productos({
  ...
  const save = async () => {
    ...
    const item = {
      ...
      stock: nuevoStock,
      unidad: form.unidad,
      tipoInventario: form.tipoInventario || 'agua_granel',
    ...
    return React.createElement(Card, {
      ...
      onClick: () => {
        setForm({
          ...
          tipoInventario: p.tipoInventario || 'agua_granel',
          motivo: ''
        });
        setExpandedId(null);
      },
  ...
  }))), React.createElement(Lbl, null, "Tipo de inventario"), React.createElement('select', { value: form.tipoInventario || 'agua_granel', onChange: e => setForm(f => ({ ...f, tipoInventario: e.target.value })), style: { width: '100%', padding: 9, borderRadius: 8, border: '1px solid var(--line-strong)', background: 'var(--surface)', color: 'var(--ink)', marginBottom: 10 } }, React.createElement('option', { value: 'agua_granel' }, 'Agua a granel · medidor y conciliación'), React.createElement('option', { value: 'producto_comercial' }, 'Producto comercial · inventario convencional')), React.createElement(Lbl, null, "Unidad"), React.createElement(Inp, {
    value: form.unidad,
    onChange: e => setForm(f => ({
      ...f,

function App() {
  ...
  if (currentUser.accesoBloqueado || !['admin', 'repartidor'].includes(currentUser.role)) return React.createElement('div', { style: { minHeight: '100vh', padding: 28, display: 'grid', placeItems: 'center', background: 'var(--bg)', color: 'var(--ink)' } }, React.createElement('div', { style: { maxWidth: 360, textAlign: 'center' } }, React.createElement('h2', { style: { marginBottom: 10 } }, 'Rol no habilitado'), React.createElement('p', { style: { color: 'var(--ink-soft)', lineHeight: 1.5, marginBottom: 16 } }, 'Esta versión solo opera con los roles ADMIN y REPARTIDOR. Solicita a la Empresa Administrativa que actualice tu cuenta.'), React.createElement('button', { onClick: () => auth.signOut(), style: { padding: '11px 16px', border: 0, borderRadius: 8, background: 'var(--accent)', color: 'var(--ink)', fontWeight: 800 } }, 'Cerrar sesión')));

...
function RepartidoresPanel({
  ...
  const cerrarRuta = async r => {
    const pendientesOffline = typeof appVentasPendientesRuta === 'function' ? await appVentasPendientesRuta(r.id) : { total: 0 };
  ...
  const guardarVentaRapida = async () => {
    ...
    try {
      ...
      const resultado = await appGuardarVentaTransferencia({
  ...
  return React.createElement(React.Fragment, null, React.createElement("div", {
  ...
  }, "Repartidores, vehículos y cargas"), msg && React.createElement("div", {
    style: {
      background: 'var(--ok-bg)',
      borderRadius: 8,
      padding: '8px 12px',
  ...
  }, [['activas', 'Cargas activas'], ['comprobantes', 'Comprobantes'], ['historial', 'Historial']].map(([v, l]) => React.createElement("button", {
    key: v,
    onClick: () => setTab(v),
    style: {
      flex: 1,
  ...
  }, "Las ventas de productos descuentan exclusivamente del saldo de la carga activa."), misRutas.length === 0 && React.createElement("div", {
    style: {
      textAlign: 'center',
      color: 'var(--ink-faint)',
      padding: '20px 0'
    ...
    }, "🖨️ Imprimir"), false && React.createElement("button", {
      onClick: () => {
  ...
  }, 'Venta registrada en la operación'), false && ventaRapida.cliente.telefono && React.createElement("button", {

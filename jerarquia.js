...
function JerarquiaPanel({ clientes = [], localidades = [], vehiculos = [], medidores = [], currentUser = {} }) {
  ...
  const clientesFiltrados = clientes.filter(cliente => {
    ...
    const texto = `${cliente.nombre || ''} ${localidadDeCliente(cliente)}`.toLowerCase();
    ...
    return React.createElement('div', { key: localidad.id, className: 'fx-assignment-row', style: { borderBottom: '1px solid var(--line)', padding: '10px 0', marginBottom: 6 } },
      React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'flex-start' } },
        React.createElement('div', null,
          ...
          React.createElement('div', { className: 'fx-coverage-assignment-id', style: { fontSize: 10, color: 'var(--ink-faint)', marginTop: 3 } }, 'ID localidad: ', localidad.id)
        ),
  ...
  return React.createElement('div', { className: 'fx-page-hierarchy fx-page-coverage', style: { padding: '14px 12px 28px' } },
    React.createElement('div', { className: 'fx-coverage-heading', style: { marginBottom: 16 } },
    ...
    mensaje && React.createElement('div', { className: 'fx-coverage-message', style: { background: 'var(--ok-bg)', color: 'var(--ok-text)', borderRadius: 9, padding: '10px 12px', fontSize: 12, fontWeight: 700, marginBottom: 12 } }, mensaje),
    React.createElement('div', { className: 'fx-coverage-overview', style: { display: 'grid', gap: 8, marginBottom: 18 } },
      ...
      React.createElement('div', { className: 'fx-coverage-localities', style: { background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 12, padding: 13 } }, localidadCards),
      React.createElement('div', { style: { textAlign: 'center', color: 'var(--ink-faint)', fontSize: 18 } }, '↓'),
    ...
    React.createElement('form', { className: 'fx-coverage-form', onSubmit: guardarLocalidad, style: { background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 12, padding: 14, marginBottom: 18 } },
    ...
    React.createElement('div', { className: 'fx-coverage-clients-heading', style: { fontSize: 12, fontWeight: 800, marginBottom: 8 } }, 'Asignación de clientes'),
    React.createElement('input', { className: 'fx-coverage-client-search', value: busqueda, onChange: e => setBusqueda(e.target.value), placeholder: 'Buscar cliente o localidad…', style: { width: '100%', boxSizing: 'border-box', padding: 11, borderRadius: 9, border: '1px solid var(--line-strong)', background: 'var(--surface)', color: 'var(--ink)', marginBottom: 8 } }),
    React.createElement('select', { className: 'fx-coverage-locality-filter', value: filtroLocalidad, onChange: e => setFiltroLocalidad(e.target.value), style: { width: '100%', padding: 10, marginBottom: 10, borderRadius: 8, border: '1px solid var(--line-strong)', background: 'var(--surface)', color: 'var(--ink)' } }, React.createElement('option', { value: 'todas' }, 'Todas las localidades'), React.createElement('option', { value: 'sin-localidad' }, 'Sin localidad asignada'), localidadesActivas.map(l => React.createElement('option', { key: l.id, value: l.id }, l.nombre + (l.repartidorNombre ? ' · ' + l.repartidorNombre : '')))),
    React.createElement('div', { className: 'fx-coverage-client-list' }, clientesFiltrados.slice(0, 100).map(cliente => React.createElement('div', { className: 'fx-coverage-client-row', key: cliente.id, style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '10px 0', borderBottom: '1px solid var(--line)' } }, React.createElement('div', { style: { minWidth: 0 } }, React.createElement('div', { style: { fontWeight: 700, fontSize: 13 } }, cliente.nombre || 'Cliente sin nombre'), React.createElement('div', { style: { color: 'var(--ink-soft)', fontSize: 11, marginTop: 3 } }, localidadDeCliente(cliente) || 'Sin localidad')), React.createElement('select', { value: cliente.localidadId || '', onChange: e => asignarCliente(cliente, e.target.value), style: { maxWidth: 175, padding: 7, borderRadius: 7, border: '1px solid var(--line-strong)', background: 'var(--surface)', color: 'var(--ink)', fontSize: 11 } }, localidadOptions))), clientesFiltrados.length === 0 && React.createElement('div', { style: { padding: 20, color: 'var(--ink-faint)', textAlign: 'center', fontSize: 12 } }, 'No hay clientes con ese filtro.'))
  );
}

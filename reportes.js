function Reportes({ notas = [], creditos = [], jornadas = [], localidades = [], currentUser = {} }) {
  ...
  const filasVenta = () => [['Fecha', 'Cliente', 'Localidad', 'Jornada', 'Vehículo', 'Medidor', 'Tarifa', 'Unidades', 'Litros', 'Precio unitario', 'Subtotal', 'Forma de pago'], ...ventas.map(venta => [fecha(venta.fecha), venta.clienteNombre || '', localidadVisible(venta), jornadaVisible(venta), vehiculoVisible(venta), medidorVisible(venta), venta.tarifaNombre || venta.tarifaSnapshot?.nombre || '', venta.garrafones || '', venta.litrosVendidos || '', venta.precioUnitario || venta.tarifaSnapshot?.precioUnitario || '', venta.total || 0, venta.formaPago || ''])];

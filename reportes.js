function Reportes({ notas = [], creditos = [], jornadas = [], localidades = [], servicios = [], currentUser = {} }) {
  ...
  return React.createElement('div', { className: 'fx-page-reports', style: { padding: '16px 12px', color: 'var(--ink)' } },
    React.createElement('div', { className: 'fx-reports-heading', style: { fontSize: 21, fontWeight: 800, marginBottom: 4 } }, 'Reportes'),
    React.createElement('div', { className: 'fx-reports-intro', style: { color: 'var(--ink-soft)', fontSize: 12, lineHeight: 1.45, marginBottom: 14 } }, 'Ventas, servicios medidos, tarifas y jornadas.'),
    React.createElement('div', { className: 'fx-reports-period', style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 } }, React.createElement('label', { style: { fontSize: 11, color: 'var(--ink-soft)' } }, 'Desde', React.createElement('input', { type: 'date', value: desde, onChange: e => setDesde(e.target.value), style: { display: 'block', width: '100%', marginTop: 4, padding: 9 } })), React.createElement('label', { style: { fontSize: 11, color: 'var(--ink-soft)' } }, 'Hasta', React.createElement('input', { type: 'date', value: hasta, onChange: e => setHasta(e.target.value), style: { display: 'block', width: '100%', marginTop: 4, padding: 9 } }))),
    React.createElement('div', { className: 'fx-reports-stats', style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 } }, React.createElement(StatTile, { value: operaciones.length, label: 'Operaciones', bg: 'var(--rail)', color: 'var(--rail-ink)' }), React.createElement(StatTile, { value: litros.toFixed(2) + ' L', label: 'Litros atendidos', bg: 'var(--accent)', color: 'var(--accent-ink)' }), React.createElement(StatTile, { value: unidades.toFixed(2), label: 'Garrafones', bg: 'var(--info)', color: '#fff' }), React.createElement(StatTile, { value: moneda(total), label: 'Total operativo', bg: 'var(--ok)', color: '#fff' })),
    React.createElement('div', { className: 'fx-reports-summary', style: { fontSize: 11, color: 'var(--ink-soft)', marginBottom: 12 } }, 'Efectivo registrado: ', moneda(efectivo), ' · Crédito generado: ', moneda(credito), ' · Facturado por relleno: ', moneda(facturado), ' · Créditos pendientes: ', moneda(creditos.reduce((suma, item) => suma + Number(item.saldo || 0), 0))),
    React.createElement('div', { className: 'fx-reports-actions', style: { display: 'flex', gap: 8, marginBottom: 14 } }, React.createElement('button', { className: 'fx-reports-export-primary', onClick: exportarExcel, style: { flex: 1, padding: 11, border: 0, borderRadius: 8, background: 'var(--accent)', color: 'var(--ink)', fontWeight: 800 } }, 'Exportar Excel'), React.createElement('button', { className: 'fx-reports-export-secondary', onClick: exportarCSV, style: { flex: 1, padding: 11, border: '1px solid var(--line-strong)', borderRadius: 8, background: 'var(--surface)', color: 'var(--ink)', fontWeight: 800 } }, 'Exportar CSV')),
    React.createElement(Card, { className: 'fx-reports-section fx-reports-tariffs' }, React.createElement('div', { className: 'fx-reports-section-title', style: { fontWeight: 800, marginBottom: 8 } }, 'Resumen por tarifa'), Object.values(tarifas).map((fila, indice) => React.createElement('div', { className: 'fx-report-tariff-row', key: indice, style: { borderTop: indice ? '1px solid var(--line)' : 'none', padding: '8px 0', fontSize: 11 } }, React.createElement('strong', null, fila.nombre), ' · ', fila.unidades.toFixed(2), ' unidades · ', fila.litros.toFixed(2), ' L · ', moneda(fila.subtotal), ' · Efectivo ', moneda(fila.efectivo), ' · Crédito ', moneda(fila.credito), ' · Facturado ', moneda(fila.facturado))), Object.keys(tarifas).length === 0 ? React.createElement('div', { style: { color: 'var(--ink-faint)', fontSize: 12 } }, 'No hay ventas en el periodo seleccionado.') : null),
    React.createElement(Card, { className: 'fx-reports-section fx-reports-reconciliation' }, React.createElement('div', { className: 'fx-reports-section-title', style: { fontWeight: 800, marginBottom: 8 } }, 'Control de jornadas'), jornadaCerrada.slice(0, 40).map(jornada => React.createElement('div', { className: 'fx-report-reconciliation-row', key: jornada.id, style: { borderTop: '1px solid var(--line)', padding: '8px 0', fontSize: 11 } }, React.createElement('strong', null, localidadVisible(jornada)), ' · ', jornada.repartidorNombre || 'Sin repartidor', React.createElement('div', { style: { color: 'var(--ink-soft)', marginTop: 3 } }, 'Inicial ', jornada.lecturaInicial ?? '—', ' → Final físico ', jornada.lecturaFinal ?? '—', ' · ', Number(jornada.litrosMedidos || 0).toFixed(2), ' L medidos · ', Number(jornada.litrosVendidosAcumulados ?? jornada.litrosVendidos ?? 0).toFixed(2), ' L vendidos · Diferencia ', Number(jornada.diferenciaLitros || 0).toFixed(2), ' L'))), jornadaCerrada.length === 0 ? React.createElement('div', { style: { color: 'var(--ink-faint)', fontSize: 12 } }, 'No hay jornadas cerradas todavía.') : null));
}

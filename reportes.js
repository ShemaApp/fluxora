function Reportes({
  ...
  const generarRespaldo = async () => {
    ...
    try {
      const colecciones = ['productos', 'clientes', 'notas', 'creditos', 'rutas', 'jornadas', 'devoluciones', 'inventario_historial', 'usuarios'];
      const data = {
  ...
  const enviarReportePorCorreo = () => {
    ...
    const link = `mailto:${encodeURIComponent(reporteEmail || '')}?subject=${encodeURIComponent(`Reporte de ventas — ${NEGOCIO.nombre}`)}&body=${encodeURIComponent(cuerpo)}`;
  ...
  }, excelGenerating ? 'Generando libro…' : '📊 Descargar libro Excel (.xlsx)')), false && React.createElement("div", {
    style: {
      background: 'var(--surface)',
      borderRadius: 12,
      padding: 16,

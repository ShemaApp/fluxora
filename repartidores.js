function RepartidoresPanel({ clientes = [], localidades = [], jornadas = [], vehiculos = [], medidores = [], currentUser = {} }) {
  ...
  const tarjetas = Array.from(repartidores.values()).map(grupo => {
    const localidadesGrupo = grupo.localidades.map(localidad => {
      ...
      return React.createElement('div', { key: localidad.id, style: { borderTop: '1px solid var(--line)', marginTop: 10, paddingTop: 9, fontSize: 11 } },
    ...
    return React.createElement('div', { key: grupo.id, style: { background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 12, padding: 13, marginBottom: 10 } },
      React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'flex-start' } },
        React.createElement('div', null, React.createElement('div', { style: { fontSize: 15, fontWeight: 800 } }, grupo.nombre), React.createElement('div', { style: { fontSize: 11, color: 'var(--ink-soft)', marginTop: 3 } }, grupo.localidades.length, ' localidad(es) · ', grupo.clientes, ' cliente(s) fijo(s)')),
  ...
  return React.createElement('div', { style: { padding: '16px 12px', color: 'var(--ink)' } },
    ...
    React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 } },
      ...
      React.createElement('div', { style: { background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 10, padding: 11 } }, React.createElement('div', { style: { fontSize: 10, color: 'var(--ink-faint)', fontWeight: 800 } }, 'JORNADAS ABIERTAS'), React.createElement('strong', { style: { display: 'block', fontSize: 22, marginTop: 4 } }, jornadasAbiertas.length))),
    tarjetas,
